...
public class WarehouseManager{
  ...
  public static <T> int transferItems(Container<T> source, Container<T> destination, int count) {
    ...
    List<T> snapshot = new ArrayList<>(source.getItems());
    for (T item : snapshot) {
      if (numberTransferred >= count || destination.isFull()) break;
  ...
  public static <T> Container<? extends T> findContainerById(Shelf<T> shelf, String containerId) {
    // TODO: Implement
    for (Container<? extends T> t: shelf.getContainers()){
      if (t.getContainerId().equals(containerId)){
        return t;
      }
    }
    return null;  
